...
export default function Agendamentoss({navigation}) {
  const items = [
    {
      name: "dennis",
      service: " corte",
  ...
  return (
    ...
    <SafeAreaView style={Styles.appDefault}>
      ...
      <View style={{ flexDirection: 'row', marginTop: 40, marginBottom: 40 }}>
        <ButtonAgendar text={"+ Agendar"} onPress={() => {navigation.navigate('Agendar')}} />
      </View>
      ...
      <View style={{ flexDirection: 'column', width: '100%' }}>
        <View style={{ flexDirection: 'column', width: '100%' }}>
          {items.map(item => (
            <TableRow
              key={Array.from({ length: 5 }, () => String.fromCharCode(Math.floor(Math.random() * 26) + 97)).join('')}
              title={item.name}
